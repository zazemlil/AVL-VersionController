            ...
            case '6': {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    file << currKeysCount << '\t' << std::to_string(versionControlledTime / double(1000)) << '\t'
                        ...
                        << std::to_string(std::filesystem::file_size("compAVLversions.txt") / (double)1024 / (double)currKeysCount) << '\t'
                        << std::to_string(std::filesystem::file_size("compFullVersions.txt") / (double)1024 / (double)currKeysCount) << '\n';
                ...
                file << "Операция удаления при:" << '\n';
                file << "Кол-во ключей" << '\t' << "VersionControlledTree time, ms" << '\t' 
                                                << "FullSaveTree time, ms" << '\t'
                                                << "VersionControlledTree mem, KB" << '\t' 
                                                << "FullSaveTree mem, KB" << '\n';

                currKeysCount = 100, step = 100;
                for (int i = 0; i < 9; i++) {
                    tree1 = new VersionControlledTree("compAVLversions.txt");
                    tree2 = new FullSaveTree("compFullVersions.txt");
                    std::vector<int> keys;
                    ...
                    int currIndex = rand() % (currKeysCount); 
                    ...
                    std::cout << "Время операции удаления при " << currKeysCount << " ключах:\n"
                        << "\tVersionControlledTree = " << versionControlledTime / double(1000) << " msec\n" 
                        << "\tFullSaveTree = " << fullSaveTime / double(1000) << " msec\n";
                    std::cout << "Размер файлов: \n"
                        << "\tVersionControlledTree = " << std::filesystem::file_size("compAVLversions.txt") / (double)1024 / currKeysCount << " КБ\n"
                ...
                file << "Операция выбора последней версии при:" << '\n';
                file << "Кол-во ключей" << '\t' << "VersionControlledTree time, ms" << '\t' 
                                                << "FullSaveTree time, ms" << '\t'
                                                << "VersionControlledTree mem, KB" << '\t' 
                                                << "FullSaveTree mem, KB" << '\n';

                currKeysCount = 1, step = 2000;
                tree1 = new VersionControlledTree("compAVLversions.txt");
                tree2 = new FullSaveTree("compFullVersions.txt");
                //std::vector<int> keys;
                for (int k = 0; k < 1; k++) {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    //for (int j = 0; j < currKeysCount; j++) {
                        ...
                        std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();
                        tree1->selectVersion(tree1->getLastVersionNumber(), selectedTree); // tree1->selectVersion(currKey, selectedTree);
                        std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
                        auto result = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                        versionControlledTime = result.count();
                        delete selectedTree;
                        ...
                        start = std::chrono::steady_clock::now();
                        tree2->selectVersion(tree2->getLastVersionNumber(), selectedTree); // tree2->selectVersion(currKey, selectedTree);
                        end = std::chrono::steady_clock::now();
                        result = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                        fullSaveTime = result.count();
                        delete selectedTree;
                        selectedTree = nullptr;
                    //}
                    std::cout << "Операция выбора последней версии при " << currKeysCount << " ключах:\n"
                        << "\tVersionControlledTree = " << versionControlledTime / double(1000) << " msec\n"
                        << "\tFullSaveTree = " << fullSaveTime / double(1000) << " msec\n";
                    std::cout << "Размер файлов: \n"
                        << "\tVersionControlledTree = " << std::filesystem::file_size("compAVLversions.txt") / (double)1024 / currKeysCount << " КБ\n"
                    ...
                    int old = currKeysCount;
                    currKeysCount += step;
                    for (int k = old; k < currKeysCount; k++) {
                        //int key = rand();
                        //tree1->insert(key);
                        //tree2->insert(key);
                        //keys.push_back(key);
